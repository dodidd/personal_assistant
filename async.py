...
from utils.load_split_temp import DocumentProcessor
...
time_analysis_prompt = PromptTemplate(
    ...
    template="""
    你会收到默认携带当前时间的问题，分析问题查询中是否有时间意图：
    ...
    {{
        ...
        "relative_days": 与当前时间的相对天数（如'最近3天'则为3，否则为null）,
...
query_optimize_prompt = PromptTemplate(
    ...
    请生成优化后的查询，用于文档检索系统。
    如果查询有时间范围，请依据当前时间在优化查询中体现时间上下文。
...
# 合并所有链为一个完整流程
full_rag_chain = SequentialChain(
    chains=[
        time_analysis_chain,  # 步骤1：分析时间意图
        query_optimize_chain,  # 步骤2：优化查询
        result_generation_chain  # 步骤3：生成最终回答
    ],
    input_variables=["query", "retrieved_docs"],  # 外部输入：原始查询 + 检索到的文档
    output_variables=["time_analysis_json", "optimized_query", "final_answer"],  # 最终输出
    verbose=True
)
...
rag_pipeline = None  # 全局变量

async def initialize_retrievers():
    """初始化检索器"""
    global loaded_embeddings_model, loaded_faiss_db, bm25_retriever, ensemble_retriever,rag_pipeline
    ...
    if loaded_embeddings_model is None:
        ...
        if ensemble_retriever :# and not rag_pipeline
            rag_pipeline = RAGPipeline(
                retriever=ensemble_retriever,
                full_chain=full_rag_chain,
...
# 修改rag_query函数，使用新的清晰流程
async def rag_query(request):
    """处理RAG查询请求 - 使用清晰的Pipeline"""
    try:
        # 初始化检索器（如果尚未初始化）
        ...
        use_conversation = body.get("use_conversation", True)
        ...
        print(f"   使用对话: {use_conversation}")
        print(f"{'='*50}\n")

        # 获取会话ID
        session_key = session_id if session_id != "default" else conversation_memory.get_session_key(request)


        # 使用pipeline处理查询
        result = await rag_pipeline.process_query(
            user_query=user_query,#修改
        ...
        print(f"\n{'='*50}")
        print(f"✅ 处理完成:")
        print(f"   是否检索: {result.get('retrieval_used')}")
        print(f"   检索数量: {result.get('retrieved_count')}")
        print(f"   对话轮数: {result.get('conversation_turns')}")
        print(f"{'='*50}\n")
...
def generate_summary_with_llm(query, docs):
    ...
    try:
        response = full_rag_chain.invoke({"query": prompt})# input
...
def rebuild_bm25_index():
    ...
    try:
        ...
        for item in metadata:
            ...
            content = item.get('preview', '') or item.get('page_content', '') or item.get('page_content_preview', '')
            ...
            doc_metadata = {
                ...
                'added_timestamp': item.get('added_timestamp', 0),
                'added_date': item.get('added_date', ''),

                # 时间分类
                'time_category': item.get('time_category', 'unknown'),

                # 内容特征
                'content_length': item.get('content_length', 0),
                'has_time_in_content': item.get('has_time_in_content', False),

                # 兼容旧字段
                'timestamp': item.get('timestamp', item.get('added_time', '')),
            ...
            for k, v in doc_metadata.items():
                if v not in [None, '', [], {}, 0] or k in ['added_timestamp', 'chunk_index']:
                    clean_metadata[k] = v

            doc = Document(
                page_content=content[:5000],  # 限制长度
        ...
        # 打印示例元数据
        if documents:
            print(f"   示例元数据字段: {list(documents[0].metadata.keys())}")
            print(f"   时间字段示例: added_time={documents[0].metadata.get('added_time', '无')}")
...
async def upload_file(request):
    ...
    try:
        ...
        if not chunks:
            return JSONResponse({"error": "无法提取文本内容"}, status_code=400)
        ...
        #  优化1：元数据去重（按chunk_id避免重复）
        existing_chunk_ids = {item.get('chunk_id') for item in existing_metadata if item.get('chunk_id')}
        new_metadata_items = []
        for chunk in chunks:
            chunk_id = chunk.metadata.get('chunk_id')
            if chunk_id and chunk_id in existing_chunk_ids:
                print(f"⚠️ 跳过重复chunk: {chunk_id}")
                continue  # 跳过已存在的chunk
            metadata_item = {
                'page_content': chunk.page_content,
                'preview': chunk.page_content[:200] + "..." if len(chunk.page_content) > 200 else chunk.page_content,
                **chunk.metadata  # 包含所有精简元数据
            }
            new_metadata_items.append(metadata_item)
            existing_chunk_ids.add(chunk_id)

        # 合并新元数据（避免重复追加）
        existing_metadata.extend(new_metadata_items)

        # 保存metadata
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(existing_metadata, f, ensure_ascii=False, indent=2)

        print(f"✅ 元数据已更新，共 {len(existing_metadata)} 条记录")

        global loaded_faiss_db, ensemble_retriever, bm25_retriever, rag_pipeline
        ...
        try:
            ...
            print("✅ FAISS索引已重新加载到内存")
        except Exception as e:
            print(f"❌ 重新加载FAISS失败: {e}")
            loaded_faiss_db = None
        ...
        # 步骤3：重建时间感知检索器和混合检索器
        if loaded_faiss_db and bm25_retriever:
            # 构建基础检索器
            faiss_retriever = loaded_faiss_db.as_retriever(search_kwargs={"k": 6})
            bm25_retriever.k = 6  # 确保k值一致

            # 创建时间感知检索器（与 initialize_retrievers 保持一致）
            from time_aware_retriever import TimeAwareRetriever
            time_aware_bm25 = TimeAwareRetriever(
                base_retriever=bm25_retriever,
                time_weight=0.3
            ...
            print("✅ 混合检索器和 RAG Pipeline 已刷新")
        else:
            ensemble_retriever = None
            rag_pipeline = None
            print("⚠️ 检索器刷新失败（FAISS/BM25加载异常）")

        return JSONResponse({
            "success": True,
            "filename": filename,
            "chunks_count": len(new_metadata_items),
...
def update_faiss_directly(chunks, embedding_model):
    ...
    try:
        ...
        # 记录更新前的向量数
        old_count = db.index.ntotal if hasattr(db.index, 'ntotal') else 0
        # 添加新文档
        db.add_documents(chunks)
        # 保存到磁盘
        db.save_local(FAISS_DB_PATH)
        # 记录更新后的向量数
        new_count = db.index.ntotal if hasattr(db.index, 'ntotal') else 0
        print(f"✅ 更新FAISS数据库: 新增 {new_count - old_count} 个向量，总计 {new_count}")
    except Exception as e:
        print(f"❌ 更新FAISS失败: {e}")
        # 失败时创建新的
        create_faiss_directly(chunks, embedding_model)
...
routes = [
    ...
    Route("/rag_query", endpoint=rag_query, methods=["POST"]),
