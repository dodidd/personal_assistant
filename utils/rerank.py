...
def create_and_save_metadata(chunks: list[Document], output_dir: str, metadata_file_name: str):
    ...
    for i, chunk in enumerate(chunks):
        metadata = {
            ...
            # 新增：提取时间信息
            'timestamp': chunk.metadata.get('timestamp'),  # 如果原始数据有时间
...
def append_to_metadata_file(new_chunks, metadata_file_path, start_index=0):
    """追加新记录到metadata文件"""
    try:
        # 1. 先读取现有数据
        existing_data = []
        ...
        for i, chunk in enumerate(new_chunks):
            new_entries.append({
                'chunk_id': f"chunk_{start_index + i}",  # 确保ID连续
                'source': chunk.metadata.get('source', ''),
                'page': chunk.metadata.get('page', 0),
                'timestamp': chunk.metadata.get('timestamp', ''),  # 关键
                'processed_at':chunk.metadata.get('processed_at', ''),
                'page_content_preview': chunk.page_content[:500]
            })
        ...
        # 使用临时文件避免写入中断
        temp_path = metadata_file_path + '.tmp'
        ...
        print(f"Metadata追加成功: 新增 {len(new_entries)} 条，总数 {len(combined_data)}")
        return True

    except Exception as e:
        print(f"追加metadata失败: {e}")
        # 清理临时文件
...
def add_to_faiss(new_chunks, embedding_model, faiss_db_path):
    ...
    try:
        ...
        metadata_filename = os.path.join(FAISS_DB_PATH, 'documents_metadata.json')
